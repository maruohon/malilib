    ...
    public void render(int mouseX, int mouseY, boolean selected, DrawContext context)
    {
        if (this.visible)
        {
            ...
            for (int i = 0; i < this.labels.size(); ++i)
            {
                ...
                if (this.centered)
                {
                    this.drawCenteredStringWithShadow(this.x + this.width / 2, yTextStart + i * fontHeight, this.textColor, text, context);
                }
                else
                {
                    this.drawStringWithShadow(this.x, yTextStart + i * fontHeight, this.textColor, text, context);
                }
            }
        }
    }
