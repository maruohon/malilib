...
public class PositionUtils
{
    ...
    public static BlockPos getPositionInfrontOfEntity(Entity entity, float verticalThreshold)
    {
        ...
        BlockPos pos = new BlockPos(entityX, entityY, entityZ);
        ...
        {
            return new BlockPos(entityX, Math.ceil(entity.getBoundingBox().maxY), entityZ);
        }

        double y = Math.floor(entityY + entity.getEyeHeight(entity.getPose()));
        float entityWidth = entity.getWidth();

        switch (entity.getHorizontalFacing())
        {
            case EAST:
                return new BlockPos((int) Math.ceil( entityX + entityWidth / 2),     (int) y, (int) Math.floor(entityZ));
            case WEST:
                return new BlockPos((int) Math.floor(entityX - entityWidth / 2) - 1, (int) y, (int) Math.floor(entityZ));
            case SOUTH:
                return new BlockPos((int) Math.floor(entityX), (int) y, (int) Math.ceil( entityZ + entityWidth / 2)    );
            case NORTH:
                return new BlockPos((int) Math.floor(entityX), (int) y, (int) Math.floor(entityZ - entityWidth / 2) - 1);
            default:
        }
